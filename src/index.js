...
function deleteTextNodesRecursive(where) {
    var nodeArr = Array.from(where.childNodes);

    for (var i = 0; i < nodeArr.length; i++) {
        if (nodeArr[i].nodeType === 3) {
            where.removeChild(nodeArr[i]);
        } else {
            deleteTextNodesRecursive(nodeArr[i]);
        }
    }
}
        ...
        func = function (root) {
            for (let child of root.childNodes) {
                if (child.nodeType === 3) {
                    ...
                    if (data.tags[child.tagName]) {
                        data.tags[child.tagName]++;
                    } else {
                        data.tags[child.tagName] = 1;
                    }
                    if (child.classList) {
                        for (let classes of child.classList) {
                            if (data.classes[classes]) {
                                data.classes[classes]++;
                            } else {
                                data.classes[classes] = 1;
                            }
                        }
                    }
                    if (child.childNodes) {
